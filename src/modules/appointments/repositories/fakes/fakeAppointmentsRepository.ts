...
class AppointmentsRepository implements IAppointmentsRepository {
  ...
  public async findAllInMounthProvider({ provider_id, month, year }: IFindAllInMonthProviderDTO): Promise<Appointment[]> {
  ...
  public async findAllInDayFromProvider({ provider_id, month, year, day }: IFindAllInDayProviderDTO): Promise<Appointment[]> {
    const appointments = this.appointments.filter(appointment => {
      return (
        appointment.provider_id === provider_id &&
